...
export type TGetPropertyListArgs = {
  developer_id?: string | number | null | undefined;
  country?: string;
} | void;

export const getDeveloperPropertyList = async (
  props: TGetPropertyListArgs,
): Promise<any> => {
  try {
    const { queryString } = generateQueryString(props);
    const res = await nextFetch<TResponse<any>>(
      `v1/auth/properties/developer-wise-property${queryString}&per_page=100`,
      {
        next: {
