...
import DubaiAllBlogList from "@/features/BlogList/DubaiAllBlogList";
...
export async function generateMetadata(): Promise<Metadata> {
  const data = await getSeoMeta({
    url: "https://www.silveroakglobal.com/in/exclusive-areas",
  });
...
const BlogPage = ({ params }: IBlogPageProps) => {
  const apiQuery = { country: params.country };
  return (
    <>
      <Section>
        <Section.HeroContainer className="flex min-h-[150px] flex-col items-center justify-center">
          ...
          <div className="absolute inset-0 h-full w-full bg-[linear-gradient(0deg,#1A1A1A_0%,rgba(73,73,73,0.00)_78.5%)]"></div>
      ...
      <Section.Container className="mt-12 lg:mt-20">
        <DubaiAllBlogList country={params.country} apiQuery={apiQuery} />
      </Section.Container>
    </>
  );
};
